...
启用微信支付需补齐整组凭据（`WECHAT_PAY_APP_ID`、`WECHAT_PAY_MCH_ID`、`WECHAT_PAY_API_KEY`、商户私钥、平台公钥），任一项缺失时渠道不会启用、`/api/plans` 也不会返回 `wechat`：

| 变量                                                     | 说明                                                                                           |
| -------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `WECHAT_PAY_PRIVATE_KEY` / `WECHAT_PAY_PRIVATE_KEY_PATH` | 商户私钥 PEM 内容，或指向密钥文件的路径，二选一                                                |
| `WECHAT_PAY_PUBLIC_KEY`                                  | 微信支付平台公钥 PEM，回调验签必需                                                             |
| `WECHAT_PAY_NOTIFY_URL`                                  | 支付回调地址，需外网可达且与商户平台配置一致；留空取 `{BETTER_AUTH_URL}/api/pay/notify/wechat` |
...
| 现象                                           | 排查方向                                                                                                                                                                                                                                                                                                                       |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 容器反复重启，日志含 `refusing to start`       | `env-guard` 拦下了不安全配置：日志会逐条列出原因，通常是 `BETTER_AUTH_SECRET`（仍是 `.env.example` 里那串公开 hex 或少于 32 字符）、`BETTER_AUTH_URL` 未设置、`POSTGRES_URL` 未设置，或显式配置的 `ADMIN_INITIAL_PASSWORD` 少于 10 字符。`ADMIN_INITIAL_PASSWORD` 等于示例值 `Admin@123` 只告警不阻塞——seed 会忽略它并随机生成 |
