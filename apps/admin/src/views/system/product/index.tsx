import { productDeleteApi, productDetailsApi, productListApi, ProductListApiResult, productSaveApi } from '@commons/core/api/admin/product.ts';
import { AppEditor } from '@commons/core/components';
import { Key, R } from '@commons/core/types';
import { defaultProduct, Product } from '@commons/core/types/product.ts';
...
const ProductIndexPage = () => {
    ...
    // 数据列表
    const dataTableColumns: TableColumnsType<Product> = [
        {
            key: 'title',
            dataIndex: 'title',
    ...
    const getDataList = async (): Promise<void> => {
        ...
        await productListApi(handleParams(dataTable)).then((result: R<ProductListApiResult>): void => {
    ...
    const handleDelete = async (id: Key = 0): Promise<void> => {
        ...
        productDeleteApi({ ids: ids }).then((result: R<string>): void => {
            if (result.code == '200') {
    ...
    const [entityFormModel, setEntityFormModel] = useState<Product>({});
    ...
    const showEntityFormModal = async (id: number = 0): Promise<void> => {
        if (id && id > 0) {
            ...
            await productDetailsApi({ id: id }).then((result: R<Product>): void => {
            ...
            Object.assign(entityFormModel, { ...defaultProduct });
            setEntityFormModel({ ...entityFormModel, ...defaultProduct });
            setEntityFormModalOpen(true);
        }
    };
    ...
    const handleFormSubmit = async (values: Product): Promise<void> => {
        console.log(`handleFormSubmit...`);
        console.log(values);
        productSaveApi(entityFormModel).then((result: R<string>): void => {
            if (result.code == '200') {
                setEntityFormModalOpen(false);
                dataTableRef.current.refresh();
            }
