...
`.github/workflows/ci.yml` 当前只做镜像构建与推送（push 到 main 或打 tag 时：`pnpm install` → `docker build -f tools/deploy/dockerfile` →
push ACR），**不执行上面这套校验**，请在提交前本地跑齐。`pnpm install` 会通过 `prepare`
