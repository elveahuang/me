...
- **系统设置模块 `/admin/settings`**（`app/pages/admin/settings.vue`，"系统设置"分组）：单页 + 顶部标签分区，承载平台级配置项，当前有"存储配置"与"模型供应商"。每个分区是独立组件 `app/components/AdminSettingsStorage.vue`、`AdminSettingsProviders.vue`（含各自的列表 + AdminDrawer 表单），页面按 `?tab=storage|providers` 深链切换（默认 storage）。原来独立的 `admin/storage.vue`、`admin/providers.vue` 已删除：存储配置并入 settings，附件总览独立成 `/admin/attachments`，模型供应商从"AI 资产"分组移入系统设置。新增平台级配置项时在 settings 页加一个分区组件，不要再建独立页面。
- **管理端新增/编辑统一走右侧滑出抽屉 `AdminDrawer`**（`app/components/AdminDrawer.vue`，Nuxt 自动导入）。列表页只保留列表 + 加载/错误/空态，实体表单不再内联在列表上方。已接入：`skills/tools/mcp/plans/news/bulletins/agents/notifications/knowledge`，以及系统设置的存储配置与模型供应商分区（knowledge 的新建与编辑各用一个抽屉；notifications 的发送表单）。约定：`:open` 绑 `editing !== null`（或各自开关），标题 `editing?.id ? t('common.edit') : t('adminForm.<x>New')`，页脚顺序 取消→保存，宽表单传 `width-class="sm:max-w-2xl"`；抽屉 slot 内的 `editing` 必须可选链（`editing?.id`），vue-tsc 会对未打开的 slot 体也做类型检查。`conversations/orders/users/index` 无新增/编辑表单，会话详情是只读侧栏面板，不套用抽屉。
