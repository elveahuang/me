...
for (const envPath of ['.env', '.env.local', '../../.env', '../../.env.local']) {
    ...
    if (existsSync(full)) {
        try {
            ...
            // 格式损坏的 .env 被静默忽略会让人以为「变量明明设了却没生效」
            console.warn(`[seed] 加载 ${full} 失败，已忽略:`, (error as Error)?.message || error);
        }
    }
}
...
async function main() {
    ...
    // 纯函数、无副作用：seed 在终端里跑（NODE_ENV 通常不是 production），
    // 服务端 env-guard 启动闸门管不到它，示例口令的防线必须在 seed 自己这里
    const { isPlaceholderEnvValue } = await import('../utils/env-guard');
