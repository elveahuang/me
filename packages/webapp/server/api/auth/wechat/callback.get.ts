...
export default defineEventHandler(async (event) => {
    try {
        ...
        setHeader(event, 'set-cookie', CLEAR_STATE_COOKIE);
        // 只回显 handler 主动 createError 的 statusMessage；微信 API 原文与未知异常可能带内部细节，一律泛化并记日志
        const isExpected = typeof e?.statusCode === 'number' && typeof e?.statusMessage === 'string';
        if (!isExpected) console.error('[wechat/callback] 登录处理失败:', e);
        setResponseStatus(event, isExpected ? e.statusCode : 500);
        return htmlPage(isExpected ? e.statusMessage : '微信登录失败，请稍后重试');
    }
});
