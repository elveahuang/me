    ...
    return tool({
        ...
        execute: async ({ action, providerId, modelId, skillId, toolId, kbId, mcpServerId, enabled }) => {
            ...
            if (action === 'set_model') {
                ...
                if (!model) return { error: 'set_model 需要 modelId' };
                // 与管理端 normalizeAgentModel 同一上限：任意长串直写 agents.model 只会在后续 resolveModel 才炸
                if (model.length > 100) return { error: 'modelId 过长（最多 100 字符）' };
                // providerId 缺省时按智能体当前供应商校验模型，而不是跳过校验
                const effectiveProviderId = providerId ?? agent.providerId;
                if (effectiveProviderId) {
                    const [p] = await db.select().from(providers).where(eq(providers.id, effectiveProviderId));
                    if (!p || !p.enabled) return { error: '供应商不存在或未启用' };
                    if (p.models.length && !p.models.includes(model)) {
                        return { error: `模型 ${model} 不在供应商 ${p.name} 的模型列表里` };
                    }
                }
                const patch: Record<string, unknown> = { model, updatedAt: new Date() };
                if (providerId) patch.providerId = providerId;
                await db.update(agents).set(patch).where(eq(agents.id, agentId));
                return { ok: true, model, providerId: effectiveProviderId ?? null };
            }
