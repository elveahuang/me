...
/**
 * json-render 组件目录（生成式 UI 的唯一事实来源）：
 * - 服务端：用 `catalog.prompt()` 生成系统提示词，教模型在 Markdown 中插入 ```json-render 代码块
 * - 客户端：Comark 的 json-render 插件按同一目录把代码块渲染成组件
 * 两端共同 import 本文件，保证提示词与渲染器永远一致。
 */
export const catalog = defineCatalog(schema, {
    ...
    // json-render 0.20 的 catalog 类型对 zod 实例有严格标称类型，这里放宽
} as never);
