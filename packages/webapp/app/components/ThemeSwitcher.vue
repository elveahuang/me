...
import { BRAND_PRESETS, MODE_PRESETS, presetText } from '@commons/contract';
...
const modes = computed(() =>
    MODE_PRESETS.map((preset) => ({
        ...preset,
        text: presetText(preset, locale.value),
        icon: MODE_ICONS[preset.value] ?? 'monitor',
    })),
);

const brandLabel = (preset: (typeof BRAND_PRESETS)[number]) => presetText(preset, locale.value);
</script>
...
<template>
    ...
    <div v-else class="space-y-3">
        <div>
            ...
            <!-- 三段控件走 aria-pressed 按钮语义（theme.css 依赖 [aria-pressed] 选中态），分组用 role=group 补标签 -->
            <div class="app-segmented w-full" role="group" :aria-label="t('common.theme')">
                <button
                    v-for="preset in modes"
                    :key="preset.value"
                    type="button"
