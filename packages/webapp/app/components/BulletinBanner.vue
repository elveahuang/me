...
import { extractApiError, formatDate, type BulletinRecord } from '@commons/contract';
...
async function load() {
    ...
    error.value = '';
    try {
        const res = await $fetch<{ bulletins: BulletinRecord[] }>('/api/bulletins', { query: { position: props.position } });
        bulletins.value = res.bulletins;
    } catch (e) {
        // 宣传栏是辅助信息，加载失败不阻塞主内容，也不弹全局错误
...
onMounted(() => {
    try {
        dismissed.value = JSON.parse(localStorage.getItem('ee_dismissed_bulletins') || '[]');
    } catch {
        dismissed.value = [];
    }
    void load();
    ...
    <div v-if="visible.length" class="space-y-2">
        <div v-for="item in visible" :key="item.id" :class="['app-alert', levelClass[item.level] ?? 'app-alert-info']">
            ...
            <div class="min-w-0 flex-1">
                ...
                <p v-if="item.endsAt" class="mt-1 text-[10px] opacity-70">{{ formatDate(item.endsAt) }} {{ t('bulletins.title') }}</p>
            </div>
