...
async function load(id: string) {
    ...
    error.value = '';
    try {
        // route.params 已被 vue-router 解码，%2F 会变成真正的 `/`，
        // 不编进单个路径段的话一个深链就能让页面带着登录凭证去请求别的路径
        const res = await $fetch<NewsDetailResponse>(`/api/news/${encodeURIComponent(id)}`);
...
const { data: initial } = await useAsyncData(`news-${route.params.id}`, () =>
    $fetch<NewsDetailResponse>(`/api/news/${encodeURIComponent(String(route.params.id))}`).catch(() => null),
);
...
<template>
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        <div v-else-if="error" class="app-alert app-alert-danger flex flex-wrap items-center justify-between gap-3">
            <span>{{ error }}</span>
            <button type="button" class="app-btn app-btn-outline shrink-0 !py-1.5 !text-xs" @click="load(String(route.params.id))">
                {{ t('common.retry') }}
            </button>
        </div>
        ...
        <article v-else-if="article" class="space-y-5">
            <header class="space-y-3">
                ...
                <div class="text-faint flex items-center gap-4 text-xs">
                    <span>{{ t('news.publishedAt') }}{{ t('common.colon') }}{{ formatDate(article.publishedAt || article.createdAt) }}</span>
