...
async function save() {
    ...
    // 合法 JSON 但形状不对同样会毁掉配置：服务端 normalizeMcpHeaders 会 400，但那句是中文原文，
    // 英文界面读不到可本地化的信息；tools 表单早有同款本地预检，这里补齐（数组/非字符串值一并挡）
    ...
    saving.value = true;
    try {
        // 显式列出字段：此前的 { ...form, headers } 会把编辑器原文 headersText 一起发上去
        const body = { name: form.name, url: form.url, transport: form.transport, enabled: form.enabled, headers };
        if (editing.value?.id) {
            await $fetch(`/api/admin/mcp-servers/${encodeURIComponent(editing.value.id)}`, { method: 'PATCH', body });
        } else {
...
async function listTools(s: McpItem) {
    ...
    try {
        const res = await $fetch<{ ok: boolean; message?: string; tools: McpTool[] }>(`/api/admin/mcp-servers/${encodeURIComponent(s.id)}/tools`);
...
async function toggle(s: McpItem) {
    listError.value = '';
    try {
        await $fetch(`/api/admin/mcp-servers/${encodeURIComponent(s.id)}`, { method: 'PATCH', body: { enabled: !s.enabled } });
        await load();
    } catch (e) {
...
async function remove(id: string) {
    ...
    listError.value = '';
    try {
        await $fetch(`/api/admin/mcp-servers/${encodeURIComponent(id)}`, { method: 'DELETE' });
        await load();
    } catch (e) {
        ...
        <AdminDrawer :open="editing !== null" :title="editing?.id ? t('common.edit') : t('adminForm.mcpNew')" @close="editing = null">
            <div class="space-y-3">
                <div class="grid grid-cols-2 gap-3">
                    <input
                        :aria-label="t('adminForm.mcpNamePlaceholder')"
                        v-model="form.name"
                        :placeholder="t('adminForm.mcpNamePlaceholder')"
                        class="app-input"
                    />
                    <select v-model="form.transport" :aria-label="t('adminForm.mcpTransport')" class="app-input">
                        <option value="http">Streamable HTTP</option>
                        <option value="sse">SSE</option>
                    </select>
                </div>
                <input
                    :aria-label="t('adminForm.mcpUrlPlaceholder')"
                    v-model="form.url"
                    :placeholder="t('adminForm.mcpUrlPlaceholder')"
                    class="app-input !font-mono !text-xs"
                />
                <textarea
                    :aria-label="t('adminForm.mcpHeadersLabel')"
