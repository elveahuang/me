...
async function load() {
    ...
    try {
        ...
        if (seq !== loadSeq) return;
        // 失败时清空旧数据：否则错误提示之外还会残留上一批列表，被误读为当前筛选结果
        items.value = [];
        total.value = 0;
...
async function fetchContent(row: NewsRow) {
    ...
    contentFailed.value = false;
    try {
        const detail = await $fetch<{ content?: string }>(`/api/admin/news/${encodeURIComponent(row.id)}`);
...
async function save() {
    ...
    if (!form.title.trim()) {
        error.value = t('adminForm.newsErrTitleRequired');
        return;
    }
    ...
    try {
        ...
        if (editing.value?.id) {
            await $fetch(`/api/admin/news/${encodeURIComponent(editing.value.id)}`, { method: 'PATCH', body });
        } else {
...
async function toggleStatus(row: NewsRow) {
    try {
        await $fetch(`/api/admin/news/${encodeURIComponent(row.id)}`, {
            method: 'PATCH',
            body: { status: row.status === 'published' ? 'draft' : 'published' },
        });
        await load();
    } catch (e) {
        error.value = extractApiError(e, t('common.error'));
...
async function togglePin(row: NewsRow) {
    try {
        await $fetch(`/api/admin/news/${encodeURIComponent(row.id)}`, { method: 'PATCH', body: { pinned: !row.pinned } });
        await load();
    } catch (e) {
        error.value = extractApiError(e, t('common.error'));
    }
...
async function remove(row: NewsRow) {
    if (!confirm(t('adminForm.newsDeleteConfirm', { title: row.title }))) return;
    try {
        await $fetch(`/api/admin/news/${encodeURIComponent(row.id)}`, { method: 'DELETE' });
    ...
    <div class="space-y-6">
        ...
        <AdminDrawer :open="editing !== null" :title="editing?.id ? t('common.edit') : t('admin.addRecord')" width-class="sm:max-w-2xl" @close="editing = null">
            <div class="space-y-3">
                <div class="grid gap-3 sm:grid-cols-2">
                    <input
                        :aria-label="t('adminForm.newsTitlePlaceholder')"
                        v-model="form.title"
                        :placeholder="t('adminForm.newsTitlePlaceholder')"
                        class="app-input sm:col-span-2"
                    />
                    <input
                        :aria-label="t('adminForm.newsCategoryPlaceholder')"
                        v-model="form.category"
                        :placeholder="t('adminForm.newsCategoryPlaceholder')"
                        class="app-input"
                    />
                    <input
                        :aria-label="t('adminForm.newsTagsPlaceholder')"
                        v-model="form.tagsText"
                        :placeholder="t('adminForm.newsTagsPlaceholder')"
                        class="app-input"
                    />
                    ...
                    <textarea
                        :aria-label="t('adminForm.newsSummaryPlaceholder')"
                        v-model="form.summary"
                        rows="2"
                        :placeholder="t('adminForm.newsSummaryPlaceholder')"
                        class="app-input sm:col-span-2"
                    />
                    <div class="sm:col-span-2">
                        <textarea
                            :aria-label="t('adminForm.newsContentPlaceholder')"
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <select v-model="form.status" :aria-label="t('common.status')" class="app-input !w-auto">
                        <option value="draft">{{ t('adminForm.statusDraft') }}</option>
                        <option value="published">{{ t('adminForm.statusPublished') }}</option>
                    </select>
                    <input v-model="form.publishedAt" type="date" :aria-label="t('news.publishedAt')" class="app-input !w-auto" />
                    <label class="text-soft flex items-center gap-2 text-sm">
                        <input v-model="form.pinned" type="checkbox" class="app-checkbox" />
                        <span>{{ t('adminForm.pin') }}</span>
                    </label>
                </div>
            </div>
            <template #footer>
        ...
        <div class="app-card p-6">
            <div class="mb-4 flex flex-wrap items-center gap-3">
                <input v-model="keyword" :placeholder="t('common.search')" :aria-label="t('common.search')" class="app-input !w-48 !py-1.5 !text-xs" />
                <select v-model="status" :aria-label="t('common.status')" class="app-input !w-auto !py-1.5 !text-xs">
            ...
            <div v-else class="overflow-x-auto">
                <table class="app-table">
                    <thead>
                        <tr>
                            <th>{{ t('adminForm.colTitle') }}</th>
                            <th>{{ t('adminForm.colCategory') }}</th>
                            <th>{{ t('common.status') }}</th>
                            <th>{{ t('adminForm.colReads') }}</th>
                            <th>{{ t('adminForm.colPublishedAt') }}</th>
                    ...
                    <tbody>
                        <tr v-for="row in items" :key="row.id">
                            ...
                            <td>
                                ...
                                >
                                    {{ row.status === 'published' ? t('adminForm.statusPublished') : t('adminForm.statusDraft') }}
                                </button>
                            </td>
                            ...
                            <td>
                                <div class="app-table-actions">
                                    <button class="app-btn app-btn-ghost app-btn-sm" @click="togglePin(row)">
                                        {{ row.pinned ? t('adminForm.unpin') : t('adminForm.pin') }}
                                    </button>
