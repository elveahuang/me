...
async function load() {
    ...
    error.value = '';
    try {
        const res = await $fetch<BulletinListResponse>('/api/admin/bulletins');
        items.value = res.bulletins;
    } catch (e) {
        // 失败时清空列表：错误横幅与列表是两条独立分支（v-if="error" / v-else-if="items.length"），
        // 清掉后才会显示空态而不是「删掉的宣传栏还在」
        items.value = [];
...
async function save() {
    ...
    if (!form.title.trim()) {
        error.value = t('adminForm.bulletinTitleRequired');
        return;
    }
    saving.value = true;
    error.value = '';
    try {
        ...
        if (editing.value?.id) {
            await $fetch(`/api/admin/bulletins/${encodeURIComponent(editing.value.id)}`, { method: 'PATCH', body });
        } else {
...
async function toggleEnabled(row: BulletinRecord) {
    try {
        await $fetch(`/api/admin/bulletins/${encodeURIComponent(row.id)}`, { method: 'PATCH', body: { enabled: !row.enabled } });
        await load();
    } catch (e) {
        error.value = extractApiError(e, t('common.error'));
    }
...
async function remove(row: BulletinRecord) {
    if (!confirm(t('adminForm.bulletinDeleteConfirm', { title: row.title }))) return;
    try {
        await $fetch(`/api/admin/bulletins/${encodeURIComponent(row.id)}`, { method: 'DELETE' });
    ...
    <div class="space-y-6">
        <div class="app-page-header !mb-0">
            <div>
                ...
                <p class="app-page-subtitle">{{ t('adminForm.bulletinSubtitle') }}</p>
            </div>
        ...
        <AdminDrawer :open="editing !== null" :title="editing?.id ? t('common.edit') : t('admin.addRecord')" width-class="sm:max-w-2xl" @close="editing = null">
            <div class="grid gap-3 sm:grid-cols-2">
                <input
                    :aria-label="t('adminForm.bulletinTitlePlaceholder')"
                    v-model="form.title"
                    :placeholder="t('adminForm.bulletinTitlePlaceholder')"
                    class="app-input sm:col-span-2"
                />
                <textarea
                    :aria-label="t('adminForm.bulletinContentPlaceholder')"
                    v-model="form.content"
                    rows="2"
                    :placeholder="t('adminForm.bulletinContentPlaceholder')"
                    class="app-input sm:col-span-2"
                />
                ...
                <input
                    :aria-label="t('adminForm.bulletinLinkPlaceholder')"
                    v-model="form.linkUrl"
                    :placeholder="t('adminForm.bulletinLinkPlaceholder')"
                    class="app-input"
                />
                <input
                    :aria-label="t('adminForm.bulletinLinkTextPlaceholder')"
                    v-model="form.linkText"
                    :placeholder="t('adminForm.bulletinLinkTextPlaceholder')"
                    class="app-input"
                />
                <select v-model="form.position" :aria-label="t('adminForm.selectPosition')" class="app-input">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-muted-2 shrink-0 text-xs">{{ t('adminForm.bulletinSortLabel') }}</span>
                    <input v-model.number="form.sortOrder" type="number" :aria-label="t('adminForm.bulletinSortLabel')" class="app-input !w-20" />
                </div>
                ...
                <div class="flex items-center gap-2">
                    <span class="text-muted-2 shrink-0 text-xs">{{ t('adminForm.bulletinStartLabel') }}</span>
                    <input v-model="form.startsAt" type="date" :aria-label="t('adminForm.bulletinStartLabel')" class="app-input !w-auto" />
                </div>
                <div class="flex items-center gap-2">
                    <span class="text-muted-2 shrink-0 text-xs">{{ t('adminForm.bulletinEndLabel') }}</span>
                    <input v-model="form.endsAt" type="date" :aria-label="t('adminForm.bulletinEndLabel')" class="app-input !w-auto" />
                </div>
            </div>
            <template #footer>
