...
<template>
    <div class="space-y-6">
        <div>
            ...
            <p class="mt-1 text-xs text-slate-500">监控全平台用户会话流、大模型交互记录与智能体响应</p>
        </div>
                    ...
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr
                                ...
                                :key="c.id"
                                :class="['transition-colors hover:bg-slate-50/60', detail?.conversation?.id === c.id ? 'bg-emerald-50/40' : '']"
                            >
                                <td class="p-4">
                                ...
                                <td class="space-x-2 p-4 text-right whitespace-nowrap">
                                    <button class="font-bold text-emerald-600 hover:text-emerald-700" @click="open(c)">查看</button>
                            ...
                            <tr v-if="!conversations.length">
                                <td colspan="3" class="p-8 text-center text-xs text-slate-400">{{ t('admin.tableEmpty') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
