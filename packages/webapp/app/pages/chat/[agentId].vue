    ...
    <div class="flex h-[calc(100vh-8.5rem)] gap-4">
        ...
        <aside class="app-card hidden w-64 shrink-0 flex-col overflow-hidden md:flex">
            ...
            <ul class="flex-1 space-y-1 overflow-y-auto px-2 pb-3">
                <li v-for="c in conversations" :key="c.id" class="group relative flex items-center">
        ...
        <!-- 聊天主视窗 -->
        <section class="app-card flex min-w-0 flex-1 flex-col overflow-hidden !rounded-2xl">
            <!-- 智能体顶部信息条 -->
            ...
            <!-- 消息列表与冷启动卡片 -->
            <div ref="scrollRef" class="flex-1 space-y-4 overflow-y-auto p-4 sm:p-6" style="background-color: var(--surface-2)">
                ...
                <div v-else class="flex h-full flex-col items-center justify-center py-10 text-center">
                    ...
                    <div class="mt-6 flex w-full max-w-md flex-col gap-2.5">
                        <button
                            ...
                            :key="prompt"
                            type="button"
                            class="app-card app-card-hover text-soft px-4 py-3 text-left text-xs"
            ...
            <form class="p-3.5" style="border-top: 1px solid var(--line); background-color: var(--surface)" @submit.prevent="() => handleSubmit()">
                <div class="flex gap-2.5">
                    <input v-model="input" :placeholder="t('chat.inputPlaceholder')" class="app-input flex-1" />
                    <button
